...
def new_transaction_event(tx):
    ...
    if tx.is_valid():
        ...
        # print("******************************")
        for txin in tx.vin:
            if 'txin' not in tx_dict:
                tx_dict['txin'] = [key_to_address.PubkeyToAddress(binascii.hexlify(txin.scriptSig))]
            else:
                tx_dict['txin'].append(key_to_address.PubkeyToAddress(binascii.hexlify(txin.scriptSig)))
        tx_dict['tx_hash'] = tx.hash
        total_value = 0
        for txout in tx.vout:
            total_value = total_value + float(txout.amount)
            if 'txout' not in tx_dict:
    ...
    def got_data(self):
        while True:
            ...
            if self.ver_recv < 209:
                ...
                h = sha256(th)
                if checksum != h[:4]:
                    raise ValueError("got bad checksum %s" %
                                     repr(self.recvbuf))
            ...
            if command in self.messagemap:
                ...
                show_debug_msg("Unknown command: '" +
                               command + "' " + repr(msg))
